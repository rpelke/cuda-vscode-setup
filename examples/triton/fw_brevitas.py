...
if os.getenv("TRITON_INTERPRET") == "1":
    ...
    # If you compare this script to `fw_torch.py`, you might have noticed that `mylib.mvm` does
    # not have any gradient definition here - despite that, `opt_model(x)` works fine even though
    # torch.compile leads to building backward graphs as well.
    # This is because there are operations that stop the gradient flow and operations that differentiable.
    # The latter leads to errors, the former does not.
    # The gradient flow is stoppend, e.g., by `tensor.detach()`, type conversions (e.g., `to(torch.int32)`),
    # or to-python or to-numpy conversions. Any non-differentiable operations in the forward pass that is
    # located after such an operation, does not need a custom gradient anymore (since the gradient flow
    # has been stopped). Consequently, training is also not possible anymore beyond this point.
