...
class CustomLinear(torch.nn.Linear):
    ...
    # If you want to debug the forward function, you can set the environment variable TRITON_INTERPRET=1
    # This prevents the compilation of the neural network (see code below).
    def forward(self, x):
        # Every operation in the forward function needs to be differentiable (if training is required)
        # If not, a custom gradient function needs to be defined (as done above for 'mvm' operator).
...
if os.getenv("TRITON_INTERPRET") == "1":
    ...
    # If torch.compile is used, the first call `opt_model(x)` will build forward and backward graphs
    # using fake/meta tensors as long as one parameter requires the gradient (`requires_grad=True`).
    # If you want to prevent this behavior, use `with torch.no_grad()` or `with torch.inference_mode()`.
...
# Test backward pass
A = torch.randn(5, 7, device="cuda", requires_grad=True)
x = torch.randn(7, device="cuda", requires_grad=True)
y = torch.ops.mylib.mvm(A, x)
loss = y.pow(2).sum()
loss.backward()
