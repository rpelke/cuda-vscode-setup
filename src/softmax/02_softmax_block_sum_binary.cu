...
__global__ void softmax_block_binary_k0(
    ...
    for (; i <= blockDim.x; i *= 2) {
        ...
        if (threadIdx.x % i == 0 && !outOfBlock && !outOfMat) {
            if (i == 1)
                As[localStartElem] += __expf(As[localStartElem + i]);
            else
                As[localStartElem] += As[localStartElem + i];
        }
        __syncthreads();
    }
