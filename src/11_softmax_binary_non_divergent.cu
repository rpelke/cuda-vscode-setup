...
__global__ void softmax_binary_non_divergent_k0(int M, int N, const float *A, float *C, float *temp) { // temp holds results for across-block reduction
    ...
    int i = 1;
    for (; i <= BLOCKSIZE_10; i*=2) {
        int index = 2*i*threadIdx.y;
        bool outOfBlock = index+i >= BLOCKSIZE_10;
        bool outOfMat = blockIdx.y*BLOCKSIZE_10+index+i >= N;

        if(!outOfBlock && !outOfMat) {
            if (i == 1) As[threadIdx.x*BLOCKSIZE_10+index] += expf(As[threadIdx.x*BLOCKSIZE_10+index + i]);
            else As[threadIdx.x*BLOCKSIZE_10+index] += As[threadIdx.x*BLOCKSIZE_10+index + i];
        }
        __syncthreads();
    }
